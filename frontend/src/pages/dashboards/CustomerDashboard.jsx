    ...
    <div>
      <div className="card p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-xs font-semibold text-slate-500">Customer / Browse</div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search cafe / city / state" className="input sm:w-80" />
            <button type="button" className="btn-ghost" onClick={refreshCafes} disabled={loading}>
              Refresh
            </button>
          </div>
        </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((c) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="text-sm font-extrabold text-slate-900 group-hover:text-orange-700">{c.cafeName || 'Cafe'}</div>
                    <div className="mt-1 text-xs text-slate-600">
                      {c.city || c.state ? `${c.city || ''}${c.city && c.state ? ', ' : ''}${c.state || ''}` : 'Location not set'}
                    </div>
                    <div className="mt-3 inline-flex rounded-full bg-orange-50 px-2 py-1 text-[11px] font-semibold text-orange-700">
                      View menu
                    </div>
                  </div>
                </Link>
