    ...
    <div className="card p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button type="button" className="btn-ghost" onClick={clear} disabled={items === 0}>
            Clear
          </button>
          <Link className="btn-primary" to="/dashboard/customer">
            Browse
          </Link>
        </div>
      </div>
            ...
            {rows.map(({ item, qty }) => (
              <div key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" className="btn-ghost px-3 py-2" onClick={() => dec(item.id)}>
                      -
                    </button>
                    <div className="min-w-10 text-center text-sm font-semibold">{qty}</div>
                    <button type="button" className="btn-ghost px-3 py-2" onClick={() => inc(item.id)}>
                      +
                    </button>
                  </div>
                </div>
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 grid gap-2">
              ...
              <div className="grid gap-1">
                ...
                <select className="input" value={amenity} onChange={(e) => setAmenity(e.target.value)}>
